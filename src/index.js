...
function checksExistsUserAccount(request, response, next) {
  ...
  const findUser = users.find((usr) => usr.username === username);
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const qtodos = user.todos.length;

  if (!user.pro && qtodos > 9) {
...
function checksTodoExists(request, response, next) {
  ...
  if (!uuidValidate(id)) {
    return response
      .status(400)
      .json({ error: "id passed is not a valid uuid." });
  }
  const findUser = users.find((usr) => usr.username === username);
  ...
  const findTodo = findUser.todos.find((td) => td.id === id);
